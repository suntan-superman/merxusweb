 ...
 * Includes: FAQs, Language Detection Config, Routing Logic
 */

// Import JSON data
import voicePromptsData from './merxus_voice_bilingual_75_with_faqs_routing.json';
...
Object.keys(voicePromptsData.categories).forEach((categoryName) => {
  ...
  industries.forEach((industry) => {
    ...
    const languageConfig = industry.language_config || {};
    const routing = industry.routing || {};
    ...
    if (industry.english) {
      VOICE_PROMPT_LIBRARY.push({
        ...
        faqs: industry.english.faqs || [],
        languageConfig,
        routing,
        // Build full prompt from components
        prompt: buildPromptFromComponents(industry.english, industryName, 'en', industry.english.faqs || []),
      });
    }
    ...
    if (industry.spanish) {
      VOICE_PROMPT_LIBRARY.push({
        ...
        faqs: industry.spanish.preguntas_frecuentes || [],
        languageConfig,
        routing,
        // Build full prompt from components
        prompt: buildPromptFromComponents(industry.spanish, industryName, 'es', industry.spanish.preguntas_frecuentes || []),
      });
    }
  });
});
 ...
 */
function buildPromptFromComponents(langData, industryName, language, faqs = []) {
...
/**
 * Get routing configuration for a category and industry
 */
export function getRoutingForIndustry(category, industry) {
  const prompts = getPromptsForIndustry(category, industry);
  return prompts[0]?.routing || null;
}

/**
 * Get language config for a category and industry
 */
export function getLanguageConfigForIndustry(category, industry) {
  const prompts = getPromptsForIndustry(category, industry);
  return prompts[0]?.languageConfig || null;
}
