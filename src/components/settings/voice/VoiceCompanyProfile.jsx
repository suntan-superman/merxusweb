...
export default function VoiceCompanyProfile({ settings, onSave, saving }) {
  const [form, setForm] = useState({
    ...
    notifyEmailAddresses: settings.notifyEmailAddresses || [],
  ...
  return (
    <section className="card">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="border-t pt-4 mt-4">
          <h4 className="text-md font-semibold text-gray-900 mb-3">📧 Message Notifications</h4>
          <p className="text-sm text-gray-600 mb-4">
            Staff members will receive email notifications when callers leave voice messages. Add the email addresses where you want to receive alerts.
          </p>

          <div className="space-y-3">
            {form.notifyEmailAddresses.map((email, index) => (
              <div key={index} className="flex gap-2 items-center">
                <input
                  type="email"
                  value={email}
                  onChange={(e) => {
                    const updated = [...form.notifyEmailAddresses];
                    updated[index] = e.target.value;
                    setForm((prev) => ({ ...prev, notifyEmailAddresses: updated }));
                  }}
                ...
                <button
                  type="button"
                  onClick={() => {
                    const updated = form.notifyEmailAddresses.filter((_, i) => i !== index);
                    setForm((prev) => ({ ...prev, notifyEmailAddresses: updated }));
                  }}
            ...
            <button
              ...
              onClick={() => {
                setForm((prev) => ({
                  ...prev,
                  notifyEmailAddresses: [...prev.notifyEmailAddresses, ''],
                }));
              }}
