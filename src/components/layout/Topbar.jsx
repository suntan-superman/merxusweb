...
export default function Topbar({ onMenuClick }) {
  ...
  const getTitle = () => {
    if (location.pathname.startsWith('/merxus')) {
      return 'Merxus AI Admin Dashboard';
    } else if (companyName) {
      return companyName;
    ...
    return 'Merxus AI Dashboard';
  };

  return (
    <header className="bg-white border-b border-gray-200 px-4 md:px-6 py-4 dark:bg-slate-900 dark:border-slate-700">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          {onMenuClick && (
            <button
              onClick={onMenuClick}
              className="md:hidden p-2 text-gray-600 hover:text-gray-900 dark:text-slate-300 dark:hover:text-slate-100"
            >
          ...
          <div>
            <h1 className="text-lg md:text-xl font-semibold text-gray-900 dark:text-slate-100">
              {getTitle()}
            </h1>
          </div>
        </div>
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          <button className="p-2 text-gray-400 hover:text-gray-600 transition-colors dark:text-slate-400 dark:hover:text-slate-200">
